		...
		uint16_t port, char *mac_addr, char *src_addr, char *dst_addr, char *data)
{
	...
	struct rte_mbuf *pkts_burst[10000];
	struct rte_crypto_op *ops_burst[MAX_PKT_BURST];
	struct l2fwd_crypto_params *cparams = &port_cparams[port];
    int tx_count = 1;
	
	struct rte_mbuf *pkt = crypto_make_packet(l2fwd_pktmbuf_pool, mac_addr, src_addr, dst_addr, data);
    if(pkt == NULL) return 0;
	if(rte_crypto_op_bulk_alloc(l2fwd_crypto_op_pool, RTE_CRYPTO_OP_TYPE_SYMMETRIC, ops_burst, tx_count) != tx_count){
		printf("Allocate crypto op failed\n");
		for(int i = 0 ; i < tx_count ; i++) if(pkts_burst[i] != NULL) {
			rte_pktmbuf_free(pkts_burst[i]);
		}
		return 0;
	}

	int retval = l2fwd_simple_crypto_enqueue(pkt, ops_burst[0], cparams);
    
	uint32_t sum_nb_rx = 0;
	do {
		uint16_t nb_rx = rte_cryptodev_dequeue_burst(cparams->dev_id, cparams->qp_id,
							ops_burst, tx_count);
		if(unlikely(nb_rx == 0)) continue;
		sum_nb_rx += nb_rx;
		for(int j = 0 ; j < nb_rx ; j++) {
			pkts_burst[j] = ops_burst[j]->sym->m_src;
			rte_crypto_op_free(ops_burst[j]);
		}
		if(port == 0) 
			ret = rte_eth_tx_burst(port, 0, pkts_burst, (uint16_t)nb_rx);
...
l2fwd_main_loop(struct l2fwd_crypto_options *options, char *mac_addr, char *src_addr, char *dst_addr, char *data)
{
    ...
    for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
	    l2fwd_send_burst(&lcore_queue_conf[lcore_id], qconf->pkt_buf[portid].len, portid, mac_addr, src_addr, dst_addr, data);
