...
static int port_init(uint16_t port, struct rte_ether_addr *my_addr){
...
void receive_packet(char *mac_addr, char *src_addr, char *dst_addr, char *data, char (*total_data)[10000]){
    uint16_t port;
    int i;
    const int header_len = 42;
    struct rte_mbuf *pkts_burst[MAX_PKTS];
    
    RTE_ETH_FOREACH_DEV(port) {
        uint32_t nb_rx = rte_eth_rx_burst(port, 0, pkts_burst, MAX_PKTS); 
        if(unlikely(nb_rx) == 0) continue;

        for(i = 0 ; i < nb_rx ; i++){
            if(pkts_burst[i] == NULL) break;
            rte_memcpy(total_data[i], rte_pktmbuf_mtod_offset(pkts_burst[i], char *, 42), pkts_burst[i]->data_len);
            
            struct rte_mbuf *m = pkts_burst[i];
            int idx = pkts_burst[i]->data_len;

            while(m->next != NULL) {
                if(idx + m->data_len >= 10000) break;
                rte_memcpy(&total_data[i][idx], rte_pktmbuf_mtod(m, void *), m->data_len);
                m = m->next;
                idx += m->data_len;
            }
        }
